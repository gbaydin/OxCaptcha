...
def single_pixel_convolution(input, x, y, kernel, kernel_width, kernel_height):
    output = 0
...
def gaussian_discrete_2d(sigma, x, y):
    g = 0
    for y_subpixel in np.arange(y - 0.5, y + 0.55, 0.1):
      for x_subpixel in np.arange(x - 0.5, x + 0.55, 0.1):
        g = g + ((1/(2*math.pi*sigma*sigma)) * math.pow(math.e, -(x_subpixel*x_subpixel+y_subpixel*y_subpixel)/(2*sigma*sigma)))
    g = g/121
    return g
...
def gaussian_2d(sigma, size):
    ...
    for j in range(size):
      for i in range(size):
        kernel[i, j] = gaussian_discrete_2d(sigma, i-(size/2), j-(size/2))
    return kernel
...
#     gaussian_kernel = gaussian_2d(sigma, kernel_size)
#     output = convolution_2d_padded(input, width, height, gaussian_kernel, kernel_size, kernel_size)
#     return output
